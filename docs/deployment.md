# Deploying Dragonhoard

A start-to-finish walkthrough for running Dragonhoard permanently on a Proxmox
LXC container under systemd. Written for someone new to Linux — every command
is explained. If you just want the short version, see "Running it" in the
top-level README.
...
2. **General**: give it a hostname like `dragonhoard`, set a root password.
...
Copy this entire `dragonhoard/` folder to `/opt/dragonhoard` on the container. The easiest way from your own machine:

```bash
scp -r dragonhoard root@<container-ip>:/opt/
```
...
```bash
chown -R dragonbot:dragonbot /opt/dragonhoard
```
...
**Important**: in the Developer Portal, under **Bot**, also enable the **Server Members Intent** toggle - the bot counts each server's human members to size its market's target stock, and Discord will refuse the connection without it.
...
```bash
cp /opt/dragonhoard/dragonhoard.service /etc/systemd/system/
```
Copies the service definition into the folder systemd scans for unit files.

Open `/etc/systemd/system/dragonhoard.service` and double check the `User=`, `WorkingDirectory=`, and `ExecStart=` lines match your actual paths (they're already set to `dragonbot` / `/opt/dragonhoard` if you followed the steps above exactly).
...
```bash
systemctl enable --now dragonhoard
```
